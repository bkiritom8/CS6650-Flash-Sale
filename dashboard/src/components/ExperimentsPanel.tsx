...
export function ExperimentsPanel() {
  ...
  const [chartData, setChartData] = useState<{id: string, data: any[]} | null>(null);
  ...
  const fetchChartData = async (id: string) => {
    try {
       ...
       if (json.data && json.data.length > 0) {
          setChartData({ id, data: json.data });
       }
    } catch (e) {
  ...
  const runExperiment = (id: string) => {
    ...
    setLogs(`Starting experiment: ${id}...\n`);
    setChartData(null); // Clear previous chart during run
    ...
    eventSource.onmessage = (event) => {
      try {
        ...
        if (msg.type === 'stdout' || msg.type === 'stderr') {
          ...
          // The script might exit with 1 due to assertion failures (e.g. oversells),
          // but the results/ CSV file is still created, so we ALWAYS attempt to fetch charts.
          fetchChartData(id);
  ...
  return (
    <div className="glass-panel p-6 flex-col gap-4 animate-fade-in" style={{ padding: '24px' }}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mt-4" style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '16px'}}>
        {EXPERIMENTS.map(exp => (
      ...
      {chartData && (
        <ExperimentChartResult id={chartData.id} data={chartData.data} />
      )}
    </div>
  );
}
